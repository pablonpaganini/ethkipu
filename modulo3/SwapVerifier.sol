...
contract SwapVerifier {
    ...
    function verify(
        ...
        // Remove liquidity
        address pair = ISimpleSwap(swapContract).getPair(tokenA, tokenB);
        IERC20(pair).approve(swapContract, liquidity);
        (uint256 aOut, uint256 bOut) = ISimpleSwap(swapContract)
            .removeLiquidity(path[0], path[1], liquidity, 0, 0, address(this), block.timestamp + 1);
